...
private LocalDate fechaInicio; // fecha en que se arrendó
private int diaCorte; // día de cobro mensual
private BigDecimal mensualidad;
private BigDecimal deposito; // garantía pactada
...
private Contrato contrato; // null si está desocupado
...
// Datos de ejemplo: Sucursal A (8 dptos), Sucursal B (15 dptos)
...
// Opcional: ocupar 1-2 deptos de ejemplo
Departamento d1 = sucursales.get(0).getDepartamentos().get(0);
d1.ocupar(new Contrato(new Inquilino("Ana", "Pérez", "0912345678", "099111222"),
LocalDate.now().minusMonths(2), 5, new BigDecimal("220.00"), new BigDecimal("100.00"), 2));
d1.getContrato().agregarDeudaArriendo(new BigDecimal("40.00"));


Departamento d2 = sucursales.get(1).getDepartamentos().get(3);
d2.ocupar(new Contrato(new Inquilino("Luis", "García", "0922334455", "098333444"),
LocalDate.now().minusMonths(1), 10, new BigDecimal("250.00"), new BigDecimal("120.00"), 3));
}
...
int dia = Math.max(1, Math.min(28, c.getDiaCorte())); // evitar días inválidos
